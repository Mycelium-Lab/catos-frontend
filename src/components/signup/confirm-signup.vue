<template>
  <div class="iphone-13-13-">
    <img class="bg-icon" alt="" src="./public/bg.svg" />

    <div class="form-registration-26">
      <div class="div8">
        <div class="text-and-button">
          <div class="text">
            <div class="checkbox-parent">
              <img class="checkbox-icon" alt="" src="./public/checkbox.svg" />

              <div class="div9">
                Подтверждаю, что имею право представлять свою организацию
              </div>
            </div>
            <div class="checkbox-parent">
              <img class="checkbox-icon" alt="" src="./public/checkbox.svg" />

              <div class="div9">
                Продолжая, я соглашаюсь, что моя организация имеет право
                оказания услуг кредиторской деятельности
              </div>
            </div>
          </div>
        </div>
        <div class="fieldsregistration-options-parent">
          <div class="fieldsregistration-options">
            <div class="fieldsregistration-options-child"></div>
            <div class="text-and-fill">
              <div class="div11">
                <span>Номер телефона </span>
                <span class="span">*</span>
              </div>
            </div>
            <div class="div12">
              <span class="txt">
                <p class="p">Укажите действующий номер телефона организации</p>
                <p class="p">для звонков и смс</p>
              </span>
            </div>
            <div class="fields-password-and-mail2">
              <div class="ru-wrapper">
                <div class="ru">RU</div>
              </div>
              <div class="iconsmobile-parent">
                <img
                  class="iconsmobile"
                  alt=""
                  src="./public/iconsmobile.svg"
                />

                <div class="alessowaitsongmailcom">+390-999-00-01</div>
              </div>
            </div>
          </div>
          <div class="fieldsregistration-options1">
            <div class="text-and-fill1">
              <div class="div11">
                <span>Электронная почта </span>
                <span class="span">*</span>
              </div>
            </div>
            <div class="div15">
              Укажите действующую почту. Она будет использована для авторизации
              и восстановления доступа
            </div>
            <div class="fields-password-and-mail3">
              <div class="iconsmail-group">
                <img class="iconsmobile" alt="" src="./public/iconsmail1.svg" />

                <div class="alessowaitsongmailcom">AlessoWaitson@gmail.com</div>
              </div>
            </div>
          </div>
          <div class="fieldsregistration-options2">
            <div class="fields-password-and-mail4">
              <div class="iconsmail-group">
                <img class="iconsmobile" alt="" src="./public/iconslock1.svg" />

                <div class="n">3345еЕN$</div>
                <img
                  class="iconspassword1"
                  alt=""
                  src="./public/iconspassword1.svg"
                />
              </div>
            </div>
            <div class="fields-password-and-mail5">
              <div class="iconsmail-group">
                <img class="iconsmobile" alt="" src="./public/iconslock2.svg" />

                <div class="n1">
                  <span class="n2">3345еЕN$ </span>
                  <span>|</span>
                </div>
                <img
                  class="iconspassword1"
                  alt=""
                  src="./public/iconspassword2.svg"
                />
              </div>
            </div>
            <div class="div16">
              <span>Придумайте пароль</span>
              <span class="span">*</span>
            </div>
            <div class="div17">
              <span>Повторите пароль </span>
              <span class="span">*</span>
            </div>
            <div class="text-and-fill1">
              <div class="div18">
                <span>Пароль </span>
                <span class="span">*</span>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="div19">
        <div class="fieldsregistration-options3">
          <div class="fieldsregistration-options-item"></div>
          <div class="text-and-fill">
            <div class="div11">
              <span>Подтверждение номера</span>
              <span class="span">*</span>
            </div>
          </div>
          <div class="fields-password-and-mail6">
            <div class="iconsmail-group">
              <img class="iconsmobile" alt="" src="./public/iconsmobile.svg" />

              <div class="div21">Введите код:</div>
            </div>
          </div>
          <div class="ruadelaryagmailcom">
            <span>Код отправлен на номер:</span>
            <span class="span6"> +7 (999) 215-43-26</span>
          </div>
          <div class="div23">
            <span>Примерное время </span>
            <span class="span7">ожидания</span>
            <span>:</span>
            <span class="span8"> </span>
            <span class="span6">1 мин</span>
          </div>
          <div class="frame-group">
            <div class="frame-container">
              <div class="iconssend-parent">
                <img
                  class="iconssend"
                  alt=""
                  src="../login/public/iconssend.svg"
                />

                <div class="div24">Отправить код повторно:</div>
              </div>
              <div class="frame">
                <div class="div25">60 сек</div>
              </div>
            </div>
            <div class="iconssend-parent">
              <img
                class="iconsmobile"
                alt=""
                src="../login/public/iconsedit1.svg"
              />

              <div class="div24">Именить номер</div>
            </div>
          </div>
          <div class="inputfields">
            <input maxlength="1" class="component-4" />
            <input maxlength="1" class="component-4" />
            <input maxlength="1" class="component-4" />
            <input maxlength="1" class="component-4" />
            <input maxlength="1" class="component-4" />
          </div>
        </div>
      </div>
      <div class="fieldsregistration-options4">
        <div class="fieldsregistration-options-item"></div>
        <div class="text-and-fill">
          <div class="div11">
            <span>Подтверждение почты</span>
            <span class="span">*</span>
          </div>
        </div>
        <div class="fields-password-and-mail6">
          <div class="iconsmail-group">
            <img class="iconsmobile" alt="" src="./public/iconsmobile.svg" />

            <div class="div21">Введите код:</div>
          </div>
        </div>
        <div class="ruadelaryagmailcom">
          <span>Код отправлен на почту:</span>
          <span class="span6"> ruadelarya@gmail.com</span>
        </div>
        <div class="div23">
          <span>Примерное время </span>
          <span class="span7">ожидания</span>
          <span>:</span>
          <span class="span8"> 1 мин</span>
        </div>
        <div class="frame-group">
          <div class="frame-container">
            <div class="iconssend-parent">
              <img
                class="iconssend"
                alt=""
                src="../login/public/iconssend.svg"
              />

              <div class="div24">Отправить код повторно:</div>
            </div>
            <div class="frame">
              <div class="div25">60 сек</div>
            </div>
          </div>
          <div class="iconssend-parent">
            <img
              class="iconsmobile"
              alt=""
              src="../login/public/iconsedit1.svg"
            />

            <div class="div24">Именить номер</div>
          </div>
        </div>
        <div class="inputfields">
          <input maxlength="1" class="component-4" />
          <input maxlength="1" class="component-4" />
          <input maxlength="1" class="component-4" />
          <input maxlength="1" class="component-4" />
          <input maxlength="1" class="component-4" />
        </div>
      </div>

      <router-link
        class="buttonnext"
        :to="{
          name:
            role === 'creditor'
              ? 'anketa-redst'
              : role === 'depositor'
              ? 'signup-depositor'
              : 'signup-borrower',
          state: { role },
        }"
      >
        <b class="b1">Продолжить</b>
      </router-link>
    </div>
    <router-link class="buttonback" to="" @click="$router.go(-1)">
      <div class="div67">Назад</div>
      <img class="search-icon" alt="" src="./public/search-icon.svg" />
    </router-link>
    <div class="header">
      <div class="page-title-parent">
        <b class="page-title1">Подтверждение регистрации</b>
      </div>
    </div>
    <div class="slidersteps">
      <div class="loader"></div>
      <div class="numbers">
        <div class="div70">из</div>
        <div class="div70">7</div>
      </div>
      <div class="steps">
        <div class="div72">2 шаг</div>
      </div>
    </div>
  </div>
</template>

<script setup lang="ts">
import { computed } from "vue";
const role = computed(() => window.history.state?.role);
</script>

<style scoped lang="scss">
.bg-icon {
  position: absolute;
  top: 0;
  left: 0;
  width: 35.19em;
  height: 20.88em;
}
.checkbox-icon {
  position: relative;
  width: 1.75em;
  height: 1.75em;
}
.div9 {
  flex: 1;
  position: relative;
  font-size: 0.75em;
  line-height: 1.33em;
  font-weight: 300;
}
.checkbox-parent,
.text,
.text-and-button {
  display: flex;
  align-items: flex-start;
  justify-content: flex-start;
}
.checkbox-parent {
  align-self: stretch;
  flex-direction: row;
  gap: 0.5em;
}
.text,
.text-and-button {
  flex-direction: column;
}
.text {
  width: 21.38em;
  gap: 0.88em;
}
.text-and-button {
  position: absolute;
  top: 29.75em;
  left: 0;
}
.fieldsregistration-options-child {
  position: absolute;
  width: 100%;
  top: calc(50% - 49.5px);
  right: 0;
  left: 0;
  border-radius: 15px;
  backdrop-filter: blur(5px);
  border: 0.5px solid rgba(46, 58, 89, 0.2);
  box-sizing: border-box;
  height: 6.5em;
}
.span {
  color: red;
}
.div11 {
  position: relative;
  font-size: 0.88em;
  line-height: 0.71em;
  font-weight: 500;
}
.text-and-fill {
  position: absolute;
  top: 0;
  left: 1.13em;
  background-color: #fff;
  display: flex;
  flex-direction: row;
  padding: 0 0.38em;
  align-items: flex-start;
  justify-content: flex-start;
}
.p {
  margin: 0;
}
.txt {
  line-break: anywhere;
  width: 100%;
}
.div12,
.ru {
  font-size: 0.63em;
}
.div12 {
  position: absolute;
  width: 84.21%;
  top: 66.97%;
  left: 6.14%;
  line-height: 120%;
  font-weight: 300;
  color: #78789a;
  display: flex;
  align-items: center;
}
.ru {
  position: relative;
  letter-spacing: 0.02em;
  line-height: 100%;
}
.ru-wrapper {
  margin: 0 !important;
  position: absolute;
  top: calc(50% - 9px);
  left: 16.19em;
  border-radius: 2px;
  border: 1px solid rgba(0, 0, 0, 0.08);
  display: flex;
  flex-direction: row;
  padding: 0.25em;
  align-items: center;
  justify-content: flex-end;
  z-index: 0;
}
.iconsmobile {
  position: relative;
  width: 1.5em;
  height: 1.5em;
  overflow: hidden;
  flex-shrink: 0;
}
.iconsmobile-parent {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0;
  align-items: center;
  justify-content: flex-start;
  gap: 0.5em;
  z-index: 1;
  color: #2e3a59;
  font-family: Inter;
}
.fields-password-and-mail2 {
  position: absolute;
  height: 36.7%;
  width: 87.72%;
  top: 22.94%;
  right: 6.14%;
  bottom: 40.37%;
  left: 6.14%;
  border-radius: 16px;
  border: 0.5px solid #656060;
  box-sizing: border-box;
  overflow: hidden;
  display: flex;
  flex-direction: column;
  padding: 0.03em 0.75em;
  align-items: flex-start;
  justify-content: center;
  gap: 0.5em;
  color: #000;
  font-family: Poppins;
}
.fieldsregistration-options {
  position: relative;
  width: 21.38em;
  height: 6.81em;
}
.div15,
.text-and-fill1 {
  position: absolute;
  display: flex;
}
.text-and-fill1 {
  top: -0.31em;
  left: 1.13em;
  background-color: #fff;
  flex-direction: row;
  padding: 0 0.38em;
  align-items: flex-start;
  justify-content: flex-start;
}
.div15 {
  width: 84.21%;
  top: 61.48%;
  left: 6.14%;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
  color: rgba(59, 59, 59, 0.58);
  align-items: center;
}
.alessowaitsongmailcom {
  flex: 1;
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.03em;
  line-height: 100%;
  font-weight: 500;
}
.iconsmail-group {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0;
  align-items: center;
  justify-content: flex-start;
  gap: 0.5em;
}
.fields-password-and-mail3 {
  position: absolute;
  height: 34.19%;
  width: 87.72%;
  top: 21.37%;
  right: 6.14%;
  bottom: 44.44%;
  left: 6.14%;
  border-radius: 16px;
  border: 0.5px solid #656060;
  box-sizing: border-box;
  overflow: hidden;
  display: flex;
  flex-direction: column;
  padding: 0.03em 0.75em;
  align-items: flex-start;
  justify-content: center;
  color: #2e3a59;
}
.fieldsregistration-options1 {
  position: relative;
  border-radius: 16px;
  border: 0.5px solid rgba(46, 58, 89, 0.2);
  box-sizing: border-box;
  width: 21.38em;
  height: 6.81em;
}
.n {
  flex: 1;
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 100%;
  font-weight: 500;
}
.iconspassword1 {
  position: relative;
  width: 1.13em;
  height: 1.13em;
  overflow: hidden;
  flex-shrink: 0;
}
.fields-password-and-mail4 {
  position: absolute;
  height: 22.35%;
  width: 87.72%;
  top: 24.02%;
  right: 6.14%;
  bottom: 53.63%;
  left: 6.14%;
  border-radius: 16px;
  background-color: #e5fee5;
  display: flex;
  flex-direction: column;
  padding: 0.03em 0.75em;
  box-sizing: border-box;
  align-items: flex-start;
  justify-content: center;
  color: #2e3a59;
}
.n2 {
  font-weight: 600;
}
.n1 {
  flex: 1;
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 100%;
}
.fields-password-and-mail5 {
  position: absolute;
  height: 22.35%;
  width: 87.72%;
  top: 66.48%;
  right: 6.14%;
  bottom: 11.17%;
  left: 6.14%;
  border-radius: 16px;
  border: 1px solid #a592dd;
  box-sizing: border-box;
  display: flex;
  flex-direction: column;
  padding: 0.03em 0.75em;
  align-items: flex-start;
  justify-content: center;
  color: #2e3a59;
}
.div16,
.div17 {
  position: absolute;
  top: 13.97%;
  left: 6.14%;
  font-size: 0.63em;
  letter-spacing: 0.4px;
  line-height: 110%;
  font-weight: 300;
}
.div17 {
  top: 56.42%;
  left: 7.02%;
}
.div18 {
  position: relative;
  font-size: 0.88em;
  letter-spacing: 1px;
  line-height: 0.71em;
  font-weight: 500;
}
.fieldsregistration-options2 {
  position: relative;
  border-radius: 16px;
  border: 0.5px solid rgba(46, 58, 89, 0.2);
  box-sizing: border-box;
  width: 21.38em;
  height: 11.81em;
}
.fieldsregistration-options-parent {
  position: absolute;
  top: 0;
  left: 0;
  display: flex;
  flex-direction: column;
  align-items: flex-end;
  justify-content: flex-start;
  gap: 1.44em;
}
.div8 {
  position: absolute;
  top: 2.5em;
  left: -22.25em;
  width: 21.38em;
  height: 39.5em;
}
.fieldsregistration-options-item {
  position: absolute;
  width: 100%;
  top: calc(50% - 103.5px);
  right: 0;
  left: 0;
  border-radius: 15px;
  backdrop-filter: blur(5px);
  border: 0.5px solid rgba(46, 58, 89, 0.4);
  box-sizing: border-box;
  height: 13.31em;
}
.div21 {
  flex: 1;
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
}
.fields-password-and-mail6 {
  position: absolute;
  width: calc(100% - 42px);
  top: calc(50% - 47.5px);
  right: 1.31em;
  left: 1.31em;
  border-radius: 16px;
  border: 0.5px solid #656060;
  box-sizing: border-box;
  height: 2.5em;
  overflow: hidden;
  display: none;
  flex-direction: column;
  padding: 0 0.75em;
  align-items: flex-start;
  justify-content: center;
  color: #2e3a59;
  font-family: Poppins;
}
.span6 {
  color: #3b3b3b;
}
.span7 {
  color: rgba(59, 59, 59, 0.68);
}
.span8 {
  color: #000;
}
.div23 {
  position: absolute;
  top: 17.81%;
  left: 6.14%;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
  color: rgba(59, 59, 59, 0.58);
}
.iconssend {
  position: relative;
  border-radius: 2px;
  width: 1.5em;
  height: 1.5em;
  overflow: hidden;
  flex-shrink: 0;
}
.div24 {
  position: relative;
  font-size: 0.75em;
  letter-spacing: 0.02em;
  line-height: 120%;
}
.iconssend-parent {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.5em;
}
.div25 {
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  background: linear-gradient(
    38.6deg,
    #2e2c2c,
    #4b4949 67.46%,
    #131313,
    rgba(0, 0, 0, 0.94),
    #434343
  );
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
}
.frame,
.frame-container {
  display: flex;
  align-items: center;
}
.frame {
  border-radius: 8px;
  background-color: rgba(253, 214, 116, 0.39);
  flex-direction: column;
  padding: 0.13em 0.69em;
  justify-content: center;
}
.frame-container {
  align-self: stretch;
  flex-direction: row;
  justify-content: space-between;
}
.frame-group {
  position: absolute;
  width: calc(100% - 42px);
  top: 8.69em;
  right: 1.31em;
  left: 1.31em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.75em;
  font-family: Poppins;
}
.div27 {
  position: relative;
  font-size: 1.25em;
  line-height: 125%;
  font-weight: 500;
}
.component-4 {
  border-radius: 15px;
  border: 1px solid #70ca98;
  box-sizing: border-box;
  width: 2.7em;
  height: 2.7em;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  outline: none;
  -webkit-tap-highlight-color: rgba(255, 255, 255, 0);
  -webkit-tap-highlight-color: transparent;
  font-weight: 600;
  font-size: 20px;
  text-align: center;
}
.inputfields {
  position: absolute;
  top: 4.19em;
  left: 1.25em;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.31em;
  color: #2e3a59;
}
.div19,
.fieldsregistration-options3 {
  position: absolute;
  top: 0;
  left: 0;
  width: 21.38em;
  height: 13.69em;
}
.div19 {
  top: 2.25em;
  left: 1.5em;
  height: 13.75em;
}
.ruadelaryagmailcom {
  position: absolute;
  top: 11.42%;
  left: 6.14%;
  font-size: 0.63em;
  line-height: 1em;
  font-weight: 300;
  color: rgba(59, 59, 59, 0.58);
}
.div38 {
  position: relative;
  font-size: 1.25em;
  line-height: 125%;
  font-weight: 500;
  display: none;
}
.component-41 {
  border-radius: 15px;
  border: 1px solid #2e3a59;
  box-sizing: border-box;
  width: 3.5em;
  height: 3.5em;
  display: flex;
  flex-direction: column;
  padding: 1em 1.38em;
  align-items: center;
  justify-content: center;
}
.div39 {
  position: relative;
  font-size: 1.25em;
  line-height: 125%;
  font-weight: 500;
  opacity: 0;
}
.component-5 {
  border-radius: 15px;
  border: 1px solid rgba(165, 146, 221, 0.5);
  box-sizing: border-box;
  width: 3.5em;
  height: 3.5em;
  display: flex;
  flex-direction: column;
  padding: 1em 1.38em;
  align-items: center;
  justify-content: center;
}
.inputfields1 {
  position: absolute;
  top: 4.19em;
  left: 1.25em;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.31em;
  color: #000;
}
.fieldsregistration-options4 {
  position: absolute;
  top: 17.44em;
  left: 1.5em;
  width: 21.38em;
  height: 13.69em;
}
.registration-options-child {
  position: absolute;
  width: 100%;
  top: calc(50% - 288px);
  right: 0;
  left: 0;
  border-radius: 15px;
  backdrop-filter: blur(5px);
  border: 0.5px solid #2e3a59;
  box-sizing: border-box;
  height: 36.25em;
}
.text-and-fill5 {
  position: absolute;
  top: 0;
  left: 1.06em;
  background-color: #fff;
  display: flex;
  flex-direction: row;
  padding: 0 0.38em;
  align-items: flex-start;
  justify-content: flex-start;
}
.span14,
.span16 {
  font-weight: 300;
}
.span16 {
  font-family: Inter;
}
.div45,
.div46 {
  position: relative;
  font-size: 0.75em;
  line-height: 0.83em;
}
.div46 {
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  font-weight: 300;
  z-index: 0;
}
.frame-inner {
  position: absolute;
  margin: 0 !important;
  top: 0;
  left: 15.31em;
  width: 1.44em;
  height: 1.56em;
  z-index: 1;
}
.parent1 {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0.5em;
  align-items: center;
  justify-content: flex-start;
  position: relative;
  gap: 0.5em;
}
.fields-password-and-mail8 {
  border-radius: 16px;
  border: 1px solid #2e3a59;
  box-sizing: border-box;
  width: 18.25em;
  height: 2.94em;
  display: flex;
  flex-direction: column;
  padding: 0.5em 0.75em;
  align-items: flex-start;
  justify-content: center;
  color: rgba(46, 58, 89, 0.36);
  font-family: Poppins;
}
.group {
  position: absolute;
  top: 2.06em;
  left: 1.5em;
  justify-content: flex-start;
  gap: 0.5em;
}
.fields-password-and-mail9,
.group,
.parent2 {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
}
.fields-password-and-mail9 {
  border-radius: 16px;
  border: 1px solid #2e3a59;
  box-sizing: border-box;
  width: 18.25em;
  height: 2.94em;
  padding: 0.5em 0.75em;
  justify-content: center;
  color: #2e3a59;
  font-family: Poppins;
}
.parent2 {
  position: absolute;
  top: 7.88em;
  left: 1.5em;
  justify-content: flex-start;
  gap: 0.5em;
}
.div50 {
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  font-weight: 300;
}
.iconsedit-outline {
  position: relative;
  width: 1.25em;
  height: 1.25em;
  overflow: hidden;
  flex-shrink: 0;
}
.parent4,
.parent6 {
  position: absolute;
  top: 13.69em;
  left: 1.5em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.5em;
}
.parent6 {
  top: 19.5em;
}
.span29 {
  letter-spacing: 0.02em;
}
.span30 {
  letter-spacing: 0.01em;
}
.div54 {
  position: relative;
  font-size: 0.88em;
  line-height: 120%;
  font-weight: 300;
}
.parent8 {
  position: absolute;
  top: 25.31em;
  left: 1.5em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.5em;
}
.catosru {
  position: relative;
  font-size: 0.88em;
  letter-spacing: 0.03em;
  line-height: 120%;
  font-weight: 300;
}
.catosru-parent,
.parent10 {
  display: flex;
  justify-content: flex-start;
  gap: 0.5em;
}
.catosru-parent {
  align-self: stretch;
  flex-direction: row;
  padding: 0.25em 0.5em;
  align-items: center;
}
.parent10 {
  position: absolute;
  top: 31.13em;
  left: 1.5em;
  flex-direction: column;
  align-items: flex-start;
}
.registration-options {
  position: absolute;
  top: 0;
  left: 0;
  width: 21.38em;
  height: 36.63em;
}
.rectangle-div,
.registration-options-inner,
.registration-options-item {
  position: absolute;
  top: 13.94em;
  left: 0.38em;
  border-radius: 20px;
  background-color: rgba(237, 244, 255, 0.3);
  width: 20.38em;
  height: 16.56em;
}
.rectangle-div,
.registration-options-inner {
  top: 1.56em;
  height: 11.38em;
}
.rectangle-div {
  top: 32em;
  height: 16.75em;
}
.registration-options-child1 {
  position: absolute;
  width: 100%;
  top: calc(50% - 395px);
  right: 0;
  left: 0;
  border-radius: 15px;
  backdrop-filter: blur(5px);
  border: 0.5px solid #2e3a59;
  box-sizing: border-box;
  height: 49.69em;
}
.business-registration-proof-container,
.div57 {
  position: relative;
  font-size: 0.75em;
  line-height: 0.83em;
  font-weight: 300;
}
.div57 {
  font-size: 0.88em;
  line-height: 0.71em;
}
.cloud-upload-outline-parent {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0.5em;
  align-items: center;
  justify-content: center;
  gap: 0.5em;
}
.fields-password-and-mail14 {
  align-self: stretch;
  border-radius: 16px;
  background-color: rgba(78, 149, 255, 0.1);
  border: 1px solid rgba(46, 58, 89, 0.24);
  box-sizing: border-box;
  height: 5.38em;
  display: flex;
  flex-direction: column;
  padding: 0.5em 0.75em;
  align-items: flex-start;
  justify-content: center;
  color: #000;
}
.business-registration-proof-parent {
  width: 18.38em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.5em;
}
.div58 {
  position: relative;
  font-size: 0.63em;
  line-height: 1em;
  font-weight: 300;
}
.iconsedit-outlinelayers-out-wrapper {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
}
.pdf {
  letter-spacing: 1px;
}
.frame-parent3,
.parent11 {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.38em;
}
.parent11 {
  flex-direction: column;
  align-items: flex-start;
  gap: 0.44em;
  color: #000;
}
.frame-parent2 {
  position: absolute;
  top: 2.75em;
  left: 1.5em;
  width: 18.38em;
}
.frame-parent2,
.frame-parent5 {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.63em;
}
.fields-password-and-mail-wrapper,
.fields-password-and-mail16 {
  align-self: stretch;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
}
.fields-password-and-mail16 {
  border-radius: 16px;
  border: 1px solid #2e3a59;
  box-sizing: border-box;
  height: 2.94em;
  padding: 0.5em 0.75em;
  justify-content: center;
}
.fields-password-and-mail-wrapper {
  justify-content: flex-start;
}
.ex-gst-tax {
  position: relative;
  font-size: 0.63em;
  line-height: 1em;
  font-weight: 300;
  font-family: Inter;
  color: #3b3b3b;
}
.frame-parent4,
.frame-parent7 {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.63em;
}
.frame-parent7 {
  align-self: stretch;
  color: rgba(46, 58, 89, 0.36);
  font-family: Poppins;
}
.frame-parent4 {
  position: absolute;
  top: 33em;
  left: 1.5em;
  width: 18.38em;
}
.fields-password-and-mail-child {
  position: absolute;
  margin: 0 !important;
  top: 0;
  left: 0;
  border-radius: 16px;
  width: 5.75em;
  height: 5.38em;
  z-index: 0;
}
.cloud-upload-outline-container {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0.5em;
  align-items: center;
  justify-content: center;
  gap: 0.5em;
  z-index: 1;
}
.fields-password-and-mail17 {
  border-radius: 16px;
  background-color: rgba(78, 149, 255, 0.1);
  border: 1px solid rgba(46, 58, 89, 0.24);
  box-sizing: border-box;
  height: 5.38em;
  overflow: hidden;
  flex-shrink: 0;
  display: flex;
  padding: 0.5em 0.75em;
  justify-content: center;
  position: relative;
  color: #fff;
}
.fields-password-and-mail17,
.operating-address-proof-parent {
  align-self: stretch;
  flex-direction: column;
  align-items: flex-start;
  gap: 0.5em;
}
.frame-parent8,
.frame-parent9,
.operating-address-proof-parent {
  display: flex;
  justify-content: flex-start;
}
.frame-parent9 {
  flex-direction: row;
  align-items: center;
  gap: 0.38em;
  color: rgba(0, 0, 0, 0.17);
}
.frame-parent8 {
  position: absolute;
  top: 16.38em;
  left: 1.5em;
  width: 18.38em;
  flex-direction: column;
  align-items: flex-start;
  gap: 0.63em;
}
.check-icon {
  position: relative;
  border-radius: 5.79px;
  width: 1.75em;
  height: 1.75em;
  overflow: hidden;
  flex-shrink: 0;
}
.check-parent,
.registration-options1 {
  position: absolute;
  left: 0;
  width: 21.38em;
}
.check-parent {
  top: 27.44em;
  display: flex;
  flex-direction: row;
  padding: 0 1.63em;
  box-sizing: border-box;
  align-items: center;
  justify-content: flex-start;
  gap: 0.94em;
  color: #000;
}
.registration-options1 {
  top: 38.88em;
  height: 50em;
}
.by-submitting-this,
.declaration {
  position: relative;
  font-size: 1.13em;
  line-height: 120%;
  font-weight: 500;
}
.by-submitting-this {
  font-size: 0.75em;
  display: flex;
  align-items: center;
  width: 21.67em;
  flex-shrink: 0;
}
.check-group {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 1.19em;
}
.frame-child1 {
  position: relative;
  border-radius: 20px;
  background-color: #0f1c15;
  width: 0.31em;
  height: 0.31em;
}
.rectangle-wrapper {
  display: flex;
  flex-direction: row;
  padding: 0.25em;
  align-items: flex-start;
  justify-content: flex-start;
}
.you-are-authorised {
  align-self: stretch;
  position: relative;
  font-size: 0.75em;
  line-height: 120%;
  font-weight: 300;
}
.you-are-authorised-to-act-on-b-wrapper {
  width: 18.63em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
}
.frame-parent11 {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  align-items: flex-end;
  justify-content: flex-start;
  gap: 0.38em;
}
.all-beneficial-owners {
  flex: 1;
  position: relative;
  font-size: 0.75em;
  line-height: 120%;
  font-weight: 300;
}
.frame-parent10,
.frame-parent12 {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.38em;
}
.frame-parent10 {
  flex-direction: column;
  gap: 0.88em;
}
.declaration-parent,
.div43 {
  position: absolute;
  width: 21.38em;
}
.declaration-parent {
  top: 91.13em;
  left: 0;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 1.25em;
}
.div43 {
  top: 2.31em;
  left: 25.25em;
  height: 102.88em;
}
.b1,
.buttonnext {
  display: flex;
  align-items: center;
  justify-content: center;
  text-decoration: none;
  color: var(--color-darkslategray-100);
  -webkit-tap-highlight-color: rgba(255, 255, 255, 0);
  -webkit-tap-highlight-color: transparent;
}
.b1 {
  position: relative;
  font-size: 1em;
  line-height: 1.75em;
  width: 6.69em;
  flex-shrink: 0;
}
.buttonnext {
  position: absolute;
  top: 33.44em;
  left: 1.5em;
  border-radius: 20px;
  background-color: #ffdb6d;
  width: 21.38em;
  height: 3em;
  overflow: hidden;
  flex-direction: row;
  padding: 0.5em 7.25em;
  box-sizing: border-box;
  text-align: center;
}
.form-registration-26 {
  position: absolute;
  top: 12.25em;
  left: 0;
  border-radius: 40px 40px 0 0;
  background-color: #fff;
  width: 24.38em;
  height: 38.88em;
}
.div67 {
  position: absolute;
  top: 0.43em;
  left: 3.29em;
  font-size: 0.88em;
  line-height: 2em;
  font-weight: 300;
}
.buttonback,
.search-icon {
  position: absolute;
  height: 2.5em;
}
.search-icon {
  top: calc(50% - 20px);
  right: 3.88em;
  border-radius: 10px;
  width: 2.5em;
  overflow: hidden;
}
.buttonback {
  top: 3.81em;
  left: 1.5em;
  border-radius: 9px;
  width: 6.38em;
  -webkit-tap-highlight-color: rgba(255, 255, 255, 0);
  -webkit-tap-highlight-color: transparent;
  color: #3b3b3b;
}
.page-title1 {
  color: #3b3b3b;
}
.page-title1,
.page-title4 {
  position: relative;
  font-size: 1.25em;
  letter-spacing: -1px;
  line-height: 1.4em;
}
.page-title-parent {
  position: absolute;
  top: 0.63em;
  left: 3em;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: center;
  gap: 7.56em;
}
.header {
  position: absolute;
  height: 5.86%;
  width: 100.51%;
  right: -0.26%;
  top: 8em;
  left: -0.26%;
  color: rgba(59, 59, 59, 0.17);
}
.battery-icon {
  position: absolute;
  height: 25.76%;
  width: 6.24%;
  top: 39.39%;
  right: 7.61%;
  bottom: 34.85%;
  left: 86.15%;
  max-width: 100%;
  overflow: hidden;
  max-height: 100%;
}
.wifi-icon {
  position: relative;
  width: 0.95em;
  height: 0.69em;
}
.mobile-signal-icon {
  position: relative;
  width: 1.06em;
  height: 0.67em;
}
.div68 {
  position: absolute;
  height: 85.71%;
  width: 100%;
  top: 9.52%;
  left: 0;
  font-size: 0.94em;
  letter-spacing: -0.02em;
  font-weight: 100;
  display: flex;
  align-items: flex-end;
  justify-content: center;
}
.time-style,
.time-style1 {
  position: absolute;
  height: 47.73%;
  width: 13.85%;
  top: 27.27%;
  right: 80.77%;
  bottom: 25%;
  left: 5.38%;
}
.time-style1 {
  display: none;
  color: #fff;
}
.ui-bars-status-bars-white {
  position: absolute;
  top: 0;
  left: 0;
  width: 24.38em;
  height: 2.75em;
  text-align: center;
  color: #1c1c1c;
}
.loader {
  position: absolute;
  top: 0.13em;
  left: 2.63em;
  border-radius: 24px;
  background-color: #f5f9ff;
  border: 1px solid rgba(0, 0, 0, 0.03);
  box-sizing: border-box;
  width: 7.69em;
  height: 1.5em;
  opacity: 0.51;
}
.div70 {
  position: relative;
  font-size: 0.5em;
  line-height: 120%;
  font-weight: 500;
}
.numbers {
  position: absolute;
  top: 0.56em;
  left: 7.19em;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.44em;
}
.div72 {
  position: relative;
  font-size: 0.75em;
  line-height: 120%;
  font-weight: 500;
}
.slidersteps,
.steps {
  position: absolute;
  height: 1.75em;
}
.steps {
  top: 0;
  left: 1.75em;
  border-radius: 19px;
  background-color: #f5f9ff;
  width: 4.06em;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  color: #2e3a59;
}
.slidersteps {
  top: 4.19em;
  left: 12.56em;
  width: 10.31em;
  color: #958463;
}
.iphone-13-13- {
  position: relative;
  background-color: #fdd674;
  width: 100%;
  height: 100vh;
  text-align: left;
  color: #3b3b3b;
  font-family: Inter;
}
</style>
