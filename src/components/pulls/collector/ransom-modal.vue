<template>
  <div class="statusapi">
    ...
    <div class="des-and-bbn_text des-and-bbn" id="desAndBbn">
      <div class="text-and-button">
        <div class="buttons-tabs">
          <div class="status">
            Информация о задолженности #124655 успешно приобретена
          </div>
        </div>
      </div>
    </div>
    ...
    <div class="des-and-bbn_bottom des-and-bbn" id="desAndBbn">
      <div class="text-and-button">
        <div
          class="buttos-tabs_action_info buttos-tabs_action buttons-tabs"
          @click="() => toLoans('debt')"
        >
          <div class="text">К выкупленным займам</div>
        </div>
      </div>
      <div class="text-and-button">
        <div class="buttos-tabs_action buttons-tabs" @click="() => toLoans()">
          <div class="text">Продолжить покупки</div>
        </div>
      </div>
    </div>
  </div>
...
const { title, subtitle, variant } = defineProps({
  title: { type: String },
...
const toLoans = (value?: any) => {
  router.push({ name: "pulls-collector", state: { curentWindow: value } });
};
...
.buttos-tabs_action {
  ...
  &_info {
    margin-bottom: 13px;
    background: rgba(87, 126, 247, 0.96);
