  ...
  <div class="iphone-13-13-ton">
    ...
    <div class="connect">
      ...
      <div class="or-press-the-the-button-below-parent" id="frameContainer10">
        <div class="or-press-the">{{ statusText }}</div>
    ...
    isWalletConnected()
      .then(response => {
        if (response.data === WalletConnectionStatusResponseEnum.successful) {
          ...
          loading.value = false;
          statusText.value = "Подключение кошелька отклонено пользователем!";
          setTimeout(() => router.push({ name: "connect-wallet" }), 3000);
        } else if (response.data === WalletConnectionStatusResponseEnum.error) {
          loading.value = false;
          statusText.value = "Ошибка: этот кошелек уже используется!";
          setTimeout(() => router.push({ name: "connect-wallet" }), 3000);
        } else if (
          response.data === WalletConnectionStatusResponseEnum.pending
        ) {
      ...
      .catch(error => {
        ...
        loading.value = false;
        statusText.value =
          "Произошла непредвиденная ошибка. Обратитесь в службу подержки!";
        setTimeout(() => router.push({ name: "connect-wallet" }), 3000);
      });
  }, 1000);
}
...
const statusText = ref("Ожидание подключения кошелька TonKeeper к сайту...");
