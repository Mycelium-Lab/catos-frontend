<template>
  <modal from="collector">
    <div class="wrapper">
      <div class="wrapper6">
        <div class="div33">Информация о задолженности #12345</div>
      </div>
      <div class="frame-parent7">
        <div class="header-wrapper">
          <div class="header1">
            <div class="div33">Основная информация</div>
          </div>
        </div>
        <div class="frame-wrapper1">
          <div class="frame-parent8">
            <div class="frame-parent1_my frame-parent1">
              <div class="creditlabel">Кредитор:</div>

              <div class="group">
                <div class="div1">Деньги до зарплаты</div>
                <img
                  class="radiobutton-icon"
                  alt=""
                  src="@/assets/images/investore.svg"
                />
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Статус:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">Не погашен</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Текущий долг:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">49 000 TON</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Ставка:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">1 % в день</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Просрочен на:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">9 дней</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Цена покупки:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">9 000 TON</div>
              </div>
            </div>
          </div>
        </div>
      </div>
      <div class="frame-parent12">
        <div class="header-wrapper">
          <div class="header1">
            <div class="div33">Параметры займа</div>
          </div>
        </div>
        <div class="frame-wrapper1">
          <div class="frame-parent8">
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Тело займа:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">40 000 TON</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Накопленный процент:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">10 000 TON</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Выплачено по долгу:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">1 000 TON</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Одобренный лимит:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">до 40 000 TON</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">На срок:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">до 30 дней</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Беспроцентный лимит:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">3 дня</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Взят:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">31.12.2023, 16.00</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Пролонгирован:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">01.02.2024, 19.20</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Выкуплен:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">01.04.2024, 19.20</div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div class="frame-parent13 frame-parent12">
        <div class="header-wrapper">
          <div class="header1">
            <div class="div33">Информация о заемщике</div>
          </div>
        </div>
        <div class="frame-wrapper1">
          <div class="frame-parent8">
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Имя:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">Иван Васильев</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Проживает в:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">Москва</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Возраст:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">37 лет</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Годовой доход:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">2 400 00 руб.</div>
              </div>
            </div>
            <div class="line-div"></div>
            <div class="frame-parent2">
              <div class="wrapper1">
                <div class="div35">Сфера деятельности:</div>
              </div>
              <div class="ton-wrapper">
                <div class="div38">Программист</div>
              </div>
            </div>

            <div class="load-wrapper" @click="() => (isLoad = true)">
              <img src="@/assets/images/iconspdf.svg" />
              <span>Скачать полную анкету</span>
              <div>
                <div class="chevron-wrapper">
                  <img src="@/assets/images/chevron-right-mini.svg" />
                </div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <div class="frame-parent28">
        <div class="frame-parent29">
          <div class="frame-child16"></div>
          <div class="buttons-tabs-group">
            <div class="buttons-tabs3">
              <div class="ton-parent">
                <div class="div68">Стоимость:</div>
                <div class="ton6">20 000 TON</div>
              </div>
              <img
                class="pie-chart-icon"
                alt=""
                src="@/assets/images/pie-chart.svg"
              />
            </div>
            <div class="frame-div">
              <div class="percent-parent">
                <img
                  class="percent-icon"
                  alt=""
                  src="@/assets/images/percent.svg"
                />

                <div class="div18">1 день = 1%</div>
              </div>
              <div class="percent-parent">
                <img
                  class="percent-icon"
                  alt=""
                  src="@/assets/images/clock.svg"
                />

                <div class="div18">3 дня = 0%</div>
              </div>
              <div class="percent-parent">
                <img
                  class="percent-icon"
                  alt=""
                  src="@/assets/images/activity-svgrepo-mini.svg"
                />

                <div class="div18">На 30 дней</div>
              </div>
            </div>
          </div>
        </div>
        <div class="des-and-bbn1">
          <div class="text-and-button-wrapper" @click="() => (isBuy = true)">
            <div class="text-and-button">
              <div class="buttons-tabs1">
                <div class="text">Реконструировать</div>
              </div>
            </div>
          </div>
          <div class="text-and-button-container">
            <div class="text-and-button" @click="toBack">
              <div class="buttons-tabs2">
                <div class="text3">Отмена</div>
              </div>
            </div>
          </div>
        </div>
      </div>

      <payment v-if="isBuy"></payment>
      <load-modal
        v-if="isLoad"
        variant="form"
        @on-close="() => (isLoad = false)"
      ></load-modal>
    </div>
  </modal>
</template>

<script setup lang="ts">
import { ref } from "vue";
import { useRouter } from "vue-router";
import payment from "@/components/pulls/borrower/payment.vue";
import catosCheckbox from "@/components/ui-kit/catos-checkbox.vue";
import loadModal from "@/components/base/load-modal.vue";
import modal from "@/components/ui-kit/modal.vue";
const isBuy = ref(false);

const isLoad = ref(false);
const router = useRouter();
const toBack = () => {
  router.go(-1);
};
</script>

<style scoped lang="scss">
.wrapper {
  height: 1800px;
}
.bg-icon,
.home-icon {
  position: absolute;
  top: 0;
  left: 0;
  width: 35.19em;
  height: 20.88em;
}
.home-icon {
  top: 0.38em;
  left: 0.75em;
  width: 1.88em;
  height: 1.88em;
  overflow: hidden;
}
.div9 {
  position: absolute;
  top: 3.6em;
  left: -3.5em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
  display: none;
}
.container1,
.iconsmenu {
  width: 3.38em;
  height: 3.38em;
}
.container1 {
  position: relative;
}
.iconsmenu {
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  z-index: 0;
  color: rgba(48, 51, 55, 0.9);
}
.basket-icon {
  position: absolute;
  top: 0.38em;
  left: 0.73em;
  width: 1.88em;
  height: 1.88em;
  overflow: hidden;
}
.div10 {
  position: absolute;
  top: 3.6em;
  left: 1.02em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
}
.iconsmenu1 {
  width: 3.38em;
  height: 3.38em;
  display: none;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  z-index: 1;
}
.news-icon {
  position: absolute;
  top: 0.38em;
  left: 0.77em;
  width: 1.88em;
  height: 1.88em;
  overflow: hidden;
}
.div11 {
  position: absolute;
  top: 3.6em;
  left: 0.2em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
}
.iconsmenu2 {
  width: 3.38em;
  height: 3.38em;
  display: none;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  z-index: 2;
}
.div12 {
  position: absolute;
  top: 3.6em;
  left: 0.5em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
}
.user-container-child {
  position: absolute;
  top: 0.16em;
  left: 0.13em;
  border-radius: 6px;
  border: 6px solid #c1c4c9;
  box-sizing: border-box;
  width: 1.56em;
  height: 1.56em;
}
.vector-icon {
  position: absolute;
  top: calc(50% - 9px);
  left: calc(50% - 9.5px);
  width: 1.19em;
  height: 1.19em;
}
.user-container {
  position: absolute;
  top: 0.41em;
  left: 0.81em;
  width: 1.81em;
  height: 1.81em;
  overflow: hidden;
}
.iconsmenu3 {
  width: 3.38em;
  height: 3.38em;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  z-index: 3;
}
.div13 {
  position: absolute;
  top: 3.6em;
  left: 0.6em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
}
.iconsmenu4 {
  width: 3.38em;
  height: 3.38em;
  display: flex;
  flex-direction: column;
  align-items: center;
  justify-content: center;
  z-index: 4;
}
.mask-icon {
  height: 85.71%;
  width: 85.71%;
  top: 7.14%;
  right: 7.14%;
  bottom: 7.14%;
  left: 7.14%;
  max-width: 100%;
  overflow: hidden;
  max-height: 100%;
}
.b1,
.mask-icon,
.status {
  position: absolute;
}
.b1 {
  height: 61.54%;
  width: 69.23%;
  top: 21.43%;
  left: 7.14%;
  font-size: 0.41em;
  line-height: 120%;
  display: inline-block;
}
.status {
  margin: 0 !important;
  top: 0.63em;
  left: 8em;
  z-index: 5;
  color: rgba(255, 255, 255, 0.96);
}
.status,
.status-icon,
.status1 {
  width: 0.88em;
  height: 0.88em;
  display: none;
}
.status1 {
  position: absolute;
  margin: 0 !important;
  top: 0.63em;
  left: 8em;
  z-index: 6;
  color: rgba(255, 255, 255, 0.96);
}
.status-icon {
  top: 0.69em;
  left: 16.75em;
  opacity: 0;
  z-index: 7;
}
.div14,
.status-icon,
.status-icon1 {
  position: absolute;
  margin: 0 !important;
}
.status-icon1 {
  top: 0.69em;
  left: 12.5em;
  width: 0.88em;
  height: 0.88em;
  display: none;
  opacity: 0;
  z-index: 8;
}
.div14 {
  top: calc(50% + 3px);
  left: 5.05em;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
  color: rgba(48, 51, 55, 0.9);
  z-index: 9;
}
.pull {
  position: absolute;
  background-color: #fff;
  width: 24.38em;
}

.pull {
  top: 0;
  left: 0;
  height: 56.88em;
  overflow: hidden;
  text-align: right;
  color: rgba(93, 104, 123, 0.9);
}
.div15 {
  align-self: stretch;
  position: relative;
  font-size: 1.13em;
  letter-spacing: 0.01em;
  line-height: 110%;
  font-weight: 600;
}
.header,
.pull-stats-child {
  position: absolute;
  box-sizing: border-box;
}
.header {
  top: 5.81em;
  left: 2.78em;
  border-radius: 7px;
  width: 18.75em;
  display: flex;
  flex-direction: column;
  padding: 0.44em 0;
  align-items: center;
  justify-content: center;
  text-align: center;
}
.pull-stats-child {
  width: 100%;
  top: calc(50% - 221.5px);
  right: 0;
  left: 0;
  border-radius: 15px;
  backdrop-filter: blur(5px);
  border: 0.5px solid rgba(46, 58, 89, 0.4);
  height: 28.25em;
}
.div16 {
  position: relative;
  font-size: 0.63em;
  line-height: 0.68em;
}
.frame {
  border-radius: 8.79px;
  border: 0.3px solid #656060;
  box-sizing: border-box;
  height: 1.25em;
  display: flex;
  flex-direction: row;
  padding: 0.42em;
  align-items: center;
  justify-content: center;
}
.filtr,
.frame-inner {
  position: absolute;
  top: 0;
}
.filtr {
  left: 0.97em;
  background-color: #fff;
  display: flex;
  flex-direction: row;
  padding: 0 0.34em;
  align-items: flex-start;
  justify-content: flex-start;
}
.frame-inner {
  left: 0;
  border-radius: 16px;
  background-color: #f6f4fc;
  width: 20.38em;
  height: 5.94em;
  overflow: hidden;
}
.div17,
.ton {
  position: relative;
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
}
.ton {
  font-size: 1.13em;
  line-height: 0.89em;
  background: linear-gradient(
    38.6deg,
    #2e2c2c,
    #4b4949 67.46%,
    #131313,
    rgba(0, 0, 0, 0.94),
    #434343
  );
}
.div17 {
  font-size: 0.63em;
  line-height: 0.68em;
  font-weight: 300;
  background: linear-gradient(#3b3b3b, #3b3b3b), #3b3b3b;
}
.ton-parent {
  position: absolute;
  top: 1.13em;
  left: 2.63em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.38em;
}
.pie-chart-icon {
  position: absolute;
  top: 1.25em;
  left: 0.69em;
  width: 1.5em;
  height: 1.5em;
  overflow: hidden;
}
.buttons-tabs,
.percent-icon {
  position: relative;
  overflow: hidden;
  flex-shrink: 0;
}
.buttons-tabs {
  border-radius: 15px;
  background-color: #fff;
  width: 10.5em;
  height: 4.06em;
}
.percent-icon {
  width: 0.75em;
  height: 0.75em;
}
.div18 {
  position: relative;
  font-size: 0.75em;
  line-height: 0.83em;
}
.percent-parent {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.5em;
}
.p {
  margin: 0;
}
.div19 {
  position: relative;
  font-size: 0.75em;
  line-height: 120%;
}
.buttons-tabs-parent,
.frame-div {
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.63em;
}
.buttons-tabs-parent {
  position: absolute;
  top: 0.94em;
  left: 0.81em;
  flex-direction: row;
  align-items: center;
  gap: 0.75em;
}
.frame-container {
  position: relative;
  width: 20.38em;
  height: 5.94em;
}
.div20 {
  position: relative;
  font-size: 0.88em;
  line-height: 0.71em;
}
.radiobutton-icon {
  position: relative;
  width: 1.75em;
  height: 1.75em;
}
.div21 {
  position: relative;
  font-size: 0.88em;
  line-height: 0.71em;
  font-weight: 300;
}
.group {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.63em;
}
.frame-wrapper {
  align-self: stretch;
  border-radius: 16px;
  border: 1px solid #f6f4fc;
  overflow: hidden;
  display: flex;
  flex-direction: column;
  padding: 0.38em 0.81em;
  align-items: flex-start;
  justify-content: center;
}
.div22 {
  position: relative;
  font-size: 0.75em;
  line-height: 0.83em;
  font-weight: 300;
}
.wrapper1 {
  width: 7.19em;
  display: flex;
  flex-direction: row;
  padding: 0.13em 0;
  box-sizing: border-box;
  align-items: flex-start;
  justify-content: flex-start;
}
.ton1 {
  flex: 1;
  position: relative;
  font-size: 0.75em;
  line-height: 0.83em;
  font-weight: 300;
}
.frame-parent2,
.ton-wrapper {
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
}
.ton-wrapper {
  padding: 0.13em 0;
}
.frame-parent2 {
  align-self: stretch;
  gap: 4.06em;
  align-items: center;
  justify-content: space-between;
}
.line-div {
  align-self: stretch;
  position: relative;
  border-top: 1px solid #f3efff;
  box-sizing: border-box;
  height: 0.06em;
}
.frame-parent1 {
  width: 18.75em;
  display: flex;
  flex-direction: column;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.5em;
}
.percent-icon1 {
  position: relative;
  width: 1em;
  height: 1em;
  overflow: hidden;
  flex-shrink: 0;
}
.div27 {
  position: relative;
  font-size: 0.63em;
  line-height: 120%;
  font-weight: 300;
  white-space: pre-wrap;
}
.frame-group,
.percent-group {
  display: flex;
  align-items: center;
  justify-content: flex-start;
}
.percent-group {
  border-radius: 16px;
  background-color: #f6f4fc;
  width: 18.75em;
  overflow: hidden;
  flex-direction: row;
  padding: 0.81em 0.75em;
  box-sizing: border-box;
  gap: 0.63em;
}
.frame-group {
  border-radius: 20px;
  flex-direction: column;
  padding: 0 0 0.75em;
  gap: 0.88em;
}
.text {
  position: relative;
  top: 0.5em;
  text-align: center;
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  font-weight: 500;
}
.buttons-tabs1 {
  align-self: stretch;
  position: relative;
  border-radius: 25px;
  background-color: rgba(87, 126, 247, 0.96);
  border: 0.5px solid rgba(87, 126, 247, 0.96);
  box-sizing: border-box;
  height: 2em;
}
.text-and-button,
.text-and-button-wrapper {
  display: flex;
  align-items: flex-start;
  justify-content: flex-start;
}
.text-and-button {
  flex: 1;
  flex-direction: column;
}
.text-and-button-wrapper {
  align-self: stretch;
  flex-direction: row;
}
.text1 {
  position: absolute;
  top: 0.5em;
  left: calc(50% - 40.5px);
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  font-weight: 500;
}
.buttons-tabs2 {
  align-self: stretch;
  position: relative;
  border-radius: 25px;
  border: 0.5px solid rgba(87, 126, 247, 0.96);
  box-sizing: border-box;
  height: 2em;
}
.text-and-button-container {
  align-self: stretch;
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
  color: #3b3b3b;
}
.des-and-bbn,
.pull-stats1 {
  display: flex;
  flex-direction: column;
  justify-content: flex-start;
}
.des-and-bbn {
  border-radius: 16px;
  background-color: #f9fbff;
  width: 18.75em;
  padding: 0.75em;
  box-sizing: border-box;
  align-items: flex-start;
  gap: 0.75em;
  text-align: right;
  color: #f8f8ff;
}
.pull-stats1 {
  position: absolute;
  top: 1.81em;
  left: 0.47em;
  align-items: center;
}
.pull-stats {
  position: absolute;
  top: 15.94em;
  left: 1.5em;
  width: 21.38em;
  height: 28.81em;
}
.frame-icon {
  position: relative;
  width: 1.5em;
  height: 1.5em;
}
.span {
  text-decoration: underline;
  color: #5d83f7;
}
.menu-2-icon {
  position: relative;
  width: 1.5em;
  height: 1.5em;
  overflow: hidden;
  flex-shrink: 0;
}
.menu-2-parent {
  justify-content: flex-start;
  gap: 0.38em;
  cursor: pointer;
}
.frame-parent5,
.menu-2-parent,
.table-svgrepo-com-2-parent {
  display: flex;
  flex-direction: row;
  align-items: center;
}
.table-svgrepo-com-2-parent {
  justify-content: flex-start;
  gap: 0.25em;
}
.frame-parent5 {
  position: absolute;
  top: 12.25em;
  left: 1.5em;
  width: 21.38em;
  justify-content: space-between;
}
.fill {
  position: absolute;
  top: 0.06em;
  left: 0.06em;
  border-radius: 20px;
  background-color: #fdd674;
  border: 0.5px solid rgba(0, 0, 0, 0.04);
  box-sizing: border-box;
  width: 8.63em;
  height: 2.06em;
}
.div31,
.div32 {
  position: absolute;
  font-size: 0.75em;
  letter-spacing: 0.01em;
  line-height: 110%;
}
.div31 {
  top: calc(50% - 6.5px);
  left: 2.75em;
  font-weight: 500;
}
.div32 {
  top: 0.83em;
  left: 12.92em;
  color: #8181a5;
}
.button-slider {
  top: 8.56em;
  left: 1.5em;
  border-radius: 20px;
  background-color: #f6f4fc;
  width: 21.38em;
  height: 2.19em;
  overflow: hidden;
  text-align: center;
}
.bg,
.button-slider,
.swipe-down {
  position: absolute;
}
.bg {
  top: 0;
  left: 0;
  background-color: rgba(166, 146, 221, 0.21);
  backdrop-filter: blur(3px);
  width: 100vw;
  height: 85.56em;
}
.swipe-down {
  top: 0.75em;
  left: 10.81em;
  border-radius: 25px;
  background-color: #cad0e4;
  width: 2.63em;
  height: 0.31em;
}
.div33 {
  position: relative;
  font-size: 0.88em;
  line-height: 130%;
}
.wrapper6 {
  position: relative;
  top: 2.5em;
  border-radius: 8px;
  display: flex;
  flex-direction: row;
  padding: 0.25em 0.75em;
  align-items: center;
  justify-content: center;
  width: 100vw;
  margin: 0 auto;
}
.header-wrapper,
.header1 {
  align-self: stretch;
  display: flex;
  align-items: center;
}
.header1 {
  border-radius: 6px;
  background-color: #f9f7fc;
  flex-direction: row;
  padding: 0.25em 0;
  justify-content: center;
}
.header-wrapper {
  flex-direction: column;
  justify-content: flex-start;
}
.div35,
.div36 {
  position: relative;
  font-size: 0.75em;
  line-height: 130%;
}
.div36 {
  font-weight: 300;
}
.parent1 {
  align-self: stretch;
  border-radius: 16px;
  overflow: hidden;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.75em;
}
.div38 {
  flex: 1;
  position: relative;
  font-size: 0.75em;
  line-height: 130%;
  font-weight: 300;
}
.frame-parent8 {
  width: 100%;
  display: flex;
  align-items: flex-start;
  gap: 0.63em;
}
.frame-parent7,
.frame-parent8,
.frame-wrapper1 {
  flex-direction: column;
  justify-content: flex-start;
}
.frame-wrapper1 {
  display: flex;
  align-items: flex-start;
  width: 77vw;
  position: relative;
  margin: 0 auto;
}
.frame-parent7 {
  display: flex;
  box-sizing: border-box;
  align-items: center;
  width: 89.5vw;
  margin: 0 auto;
  position: relative;

  top: 4.8em;

  border-radius: 14px;
  border: 1px solid #f8f6fc;

  overflow: hidden;
  padding: 1.13em 1.31em 1.5em;
  gap: 1.63em;
}
.frame-parent12,
.frame-parent7,
.header3 {
  display: flex;
  box-sizing: border-box;
  align-items: center;
}
.frame-parent12 {
  position: relative;
  top: 6.2em;
  border-radius: 14px;
  border: 1px solid #f8f6fc;
  width: 89.7vw;
  overflow: hidden;
  flex-direction: column;
  padding: 1.13em 1.31em 1.5em;
  justify-content: flex-start;
  gap: 1.63em;
  margin: 0 auto;
}
.header3 {
  border-radius: 6px;
  background-color: #f9f7fc;
  width: 100%;
  flex-direction: row;
  padding: 0.25em 0;
  justify-content: center;
}
.txt {
  line-break: anywhere;
  width: 100%;
}
.div57 {
  position: relative;
  font-size: 0.75em;
  line-height: 130%;
  color: #78789a;
  text-align: center;
  display: flex;
  align-items: center;
  width: 100%;
}
.header-and-text {
  border-radius: 6px;
  border: 1px solid #f9f7fc;
  display: flex;
  flex-direction: column;
  padding: 0 0 0.63em;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.63em;
  width: 77vw;
  position: relative;
  margin: 0 auto;
}
.span2 {
  letter-spacing: 1px;
}
.header-and-text-parent {
  position: relative;
  top: 7.7em;

  border-radius: 14px;
  border: 1px solid #f8f6fc;
  box-sizing: border-box;
  width: 89.7vw;
  overflow: hidden;
  display: flex;
  flex-direction: column;
  padding: 1.13em 1.31em 1.5em;
  align-items: center;
  justify-content: flex-start;
  margin: 0 auto;
  gap: 1.63em;
}
.frame-child16 {
  position: relative;
  top: 0;
  left: 0;
  border-radius: 16px 16px 0 0;
  background-color: #f6f4fc;
  width: 77vw;
  height: 5.94em;
  margin: 0 auto;
  overflow: hidden;
}

.div68 {
  font-size: 0.75em;
  line-height: 0.56em;
  font-weight: 300;
  color: #3b3b3b;
}
.ton6 {
  background-color: red;

  /* Create the gradient. */
  background: linear-gradient(
    38.6deg,
    #2e2c2c,
    #4b4949 67.46%,
    #131313,
    rgba(0, 0, 0, 0.94),
    #434343
  );

  /* Set the background size and repeat properties. */
  background-size: 100%;
  background-repeat: repeat;

  /* Use the text as a mask for the background. */
  /* This will show the gradient as a text color rather than element bg. */
  -webkit-background-clip: text;
  -webkit-text-fill-color: transparent;
  -moz-background-clip: text;
  -moz-text-fill-color: transparent;
  font-size: 1em;
  line-height: 0.89em;
}
.buttons-tabs3 {
  position: relative;
  border-radius: 15px;
  background-color: #fff;
  width: 42.1vw;
  height: 4.06em;
  overflow: hidden;
  flex-shrink: 0;
}
.buttons-tabs-group {
  position: absolute;
  top: 0.94em;
  left: 0.81em;
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.94em;
}
.frame-parent29 {
  align-self: stretch;
  position: relative;
  height: 5.94em;
  z-index: 1;
}
.text3 {
  position: absolute;
  top: 0.5em;
  left: calc(50% - 27px);
  font-size: 0.88em;
  letter-spacing: 0.02em;
  line-height: 120%;
  font-weight: 500;
}
.des-and-bbn1 {
  align-self: stretch;
  border-radius: 16px;
  background-color: #f9fbff;
  display: flex;
  flex-direction: column;
  padding: 1.88em 0.75em 0.75em;
  align-items: flex-start;
  justify-content: flex-start;
  gap: 0.75em;
  z-index: 0;
  margin-top: -0.94em;
  text-align: right;
  color: #f8f8ff;
  width: 77vw;
  margin: 0 auto;
  margin-top: -0.94em;
}
.frame-parent28,
.swipe-down-parent {
  position: absolute;
  box-sizing: border-box;
  overflow: hidden;
}
.frame-parent28 {
  top: 9.1em;
  border-radius: 14px;
  border: 1px solid #f8f6fc;
  width: 21.38em;
  display: flex;
  flex-direction: column;
  padding: 1.13em 1.31em 1.5em;
  align-items: center;
  justify-content: flex-start;
  position: relative;
  width: 89.7vw;
  margin: 0 auto;
}
.swipe-down-parent {
  top: 8.19em;
  left: 0;
  border-radius: 30px 30px 0 0;
  background-color: #fff;
  box-shadow: 0 12px 12px rgba(151, 71, 255, 0.04);
  border: 1px solid rgba(0, 0, 0, 0.04);
  width: 100vw;
  height: 100%;
}
.div71 {
  position: absolute;
  top: 0.43em;
  left: 3.29em;
  font-size: 0.88em;
  line-height: 2em;
  font-weight: 300;
}
.buttonback,
.search-icon {
  position: absolute;
  height: 2.5em;
}
.search-icon {
  top: calc(50% - 20px);
  right: 3.88em;
  border-radius: 10px;
  width: 2.5em;
  overflow: hidden;
}
.buttonback {
  top: 4.25em;
  left: 1.5em;
  border-radius: 9px;
  width: 6.38em;
}
.battery-icon {
  position: absolute;
  height: 25.76%;
  width: 6.24%;
  top: 39.4%;
  right: 7.61%;
  bottom: 34.85%;
  left: 86.15%;
  max-width: 100%;
  overflow: hidden;
  max-height: 100%;
}
.wifi-icon {
  position: relative;
  width: 0.95em;
  height: 0.69em;
}
.mobile-signal-icon {
  position: relative;
  width: 1.06em;
  height: 0.67em;
}
.div72 {
  position: absolute;
  height: 85.71%;
  width: 100%;
  top: 9.52%;
  left: 0;
  font-size: 0.94em;
  letter-spacing: -0.02em;
  font-weight: 100;
  display: flex;
  align-items: flex-end;
  justify-content: center;
}
.time-style,
.time-style1 {
  position: absolute;
  height: 47.73%;
  width: 13.85%;
  top: 27.27%;
  right: 80.77%;
  bottom: 25%;
  left: 5.38%;
}
.time-style1 {
  display: none;
  color: #fff;
}
.ui-bars-status-bars-white {
  position: absolute;
  top: 0;
  left: -0.03em;
  width: 24.38em;
  height: 2.75em;
  text-align: center;
  color: #1c1c1c;
}
.div8 {
  position: relative;
  background-color: #fff;
  width: 100%;
  height: 95.81em;
  overflow-x: hidden;
  text-align: left;
  color: #3b3b3b;
  font-family: Inter;
}
.frame-parent13 {
  top: 7.8em;
}
.load-wrapper {
  width: 77vw;
  position: relative;
  margin: 0 auto;
  height: 40px;
  display: flex;
  gap: 1em;
  align-items: center;
  background: rgba(165, 146, 221, 0.07);
  border-radius: 8px;
  padding: 8px 0px 8px 12px;
  margin-top: 1em;
  & span {
    font-weight: 400;
    font-size: 12px;
    line-height: 140%;
  }
}
.chevron-wrapper {
  position: absolute;
  display: flex;
  right: 0em;
  height: 2.4em;
  width: 10.26vw;
  justify-content: center;
  background: rgba(165, 146, 221, 0.14);
  border-bottom-right-radius: 8px;
  align-items: center;
  border-top-right-radius: 8px;
  top: 0em;
}
.frame-parent1 {
  display: flex;
  flex-direction: row;
  align-items: flex-start;
  justify-content: flex-start;
  align-self: stretch;
  gap: 0.75em;
  width: 77vw;
  margin: 0 auto;
}
.frame-parent1_my {
  border: 1px solid #f6f4fc;
  border-radius: 16px;
  justify-content: space-between;
  padding: 9px 13px 9px 13px;
  margin: 0em;
  align-items: center;
  height: 40px;
  width: 100%;
}
.creditlabel,
.creditoption {
  color: rgba(59, 59, 59, 1);
  font-size: 14px;
  font-weight: 400;
}
.creditoption {
  font-weight: 300;
}
.group {
  display: flex;
  flex-direction: row;
  align-items: center;
  justify-content: flex-start;
  gap: 0.63em;
}
.div1 {
  position: relative;
  font-size: 0.88em;
  line-height: 130%;
  font-weight: 300;
}
</style>
